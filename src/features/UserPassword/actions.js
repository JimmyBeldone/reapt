...
export const sendEmail = username => dispatch => {
    ...
    apiUtil(true, dispatch)
        .post(configAskPassword.apiRoute, qs.stringify({ username }))
        .then(response => {
...
export const resetPassword = (token, password) => dispatch => {
    ...
    apiUtil(true, dispatch)
        .post(
            `${`${configResetPassword.apiRoute}/${token}`}`,
        ...
        .then(response => {
            ...
            if (configResetPassword.hasResolveAction) {
                dispatch(configResetPassword.resolveAction(response));
            }
        })
        .catch(error => {
            dispatch({
...
export const changePassword = (current, password) => dispatch => {
    ...
    apiUtil(false, dispatch)
        .post(
            configChangePassword.apiRoute,
            qs.stringify({ current_password: current, password })
        )
        .then(response => {
            dispatch({
            ...
            if (configChangePassword.hasResolveAction) {
                dispatch(
                    configChangePassword.resolveAction(dispatch, response)
                );
            }
        })
        .catch(error => {
